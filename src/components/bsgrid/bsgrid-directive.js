...
angular.module('app').directive('bsGrid', function () {
    return {
        ...
        restrict: 'E',
        controller: function ($scope, $element, $filter, $timeout, $state, store, $compile, $q) {
            ...
            $scope.getLocalStorage = function () {
                ...
                var data = store.get($scope.localStorageName());
                if (data) {
            ...
            $scope.setLocalStorage = function () {
                ...
                store.set($scope.localStorageName(), {
            ...
            $scope.GetRowsForExport = function (exportType, fieldNames, csvData) {
                ...
                _.each(itemsToExport, function (itemToExport) {
                    ...
                    _.each(fieldNames, function (fieldName) {
                        ...
                        else if (!angular.isUndefined(itemToExport.search) && !angular.isUndefined(itemToExport.search[fieldName])) {
            ...
            _.forEach($scope.options.pageSizeOptions, function (size) {
                $scope.dropdownOptions.push({
                    text: size,
                    click: 'options.pageSize = size'
                });
            });
            $scope.dropdownOptions.push({divider: true});
            //Add colulum options
            _.forEach($scope.options.columns, function (col) {
                if(col.visible) {
                    $scope.dropdownOptions.push({
                        text:  '<i class="glyphicon glyphicon-ok">' + col.name,
                        click: 'toggleColumn(col)'
                    });
                } else {
                    $scope.dropdownOptions.push({
                        text: '<i class="glyphicon glyphicon-remove">' + col.name,
                        click: 'toggleColumn(col)'
                    });
                }
            });
